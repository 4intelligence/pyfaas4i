...
def _get_proxies(proxy_url: Union[str, None],
                 proxy_port: Union[str, None]) -> Union[dict, None]:
    """
    Set user proxies for making the request
    Args:
        proxy_url: A proxy for URL during the request
        proxy_port: A proxy for port to compose the URL during the request
    Returns:
        A dictionary with proxies or "None"
    """
    proxies = None
    if proxy_url and proxy_port:

        proxies = {
                'http': proxy_url + ":" + str(proxy_port),
                'https': proxy_url + ":" + str(proxy_port)
                }

    return proxies


def _version_check(proxies):
    '''
...
def _get_auth_data() -> str:
    """
    Retrieves the authentication data from the config.json file. Requires the
    user to run the pyfaas4i.faas.login function first to generate the config.json file.

    Returns:
    auth_data: Authentication data for the specified domain.
    """
    ...
    try:
        auth_data = config_json["auths"][domain]
    except:
        raise ValueError('auth_data not found. Make sure to run the pyfaas4i.faas.login function and complete the login.')
...
def download_zip(
    ...
    **kwargs
    ...
    # ---- Initialize proxy args
    proxy_url = None
    proxy_port = None

    # ---- Read kwargs

    if any([x not in ['version_check',
                      'proxy_url', 'proxy_port'] for x in list(kwargs.keys())]):
        unexpected = list(kwargs.keys())
        for arg in ['version_check',
...
class AuthenticationError(Exception):
    ...
    ''' 
    def __init__(self, msg="Status Code: 401. Content: Expired Authentication.\nPlease run 'pyfaas4i.faas.login()' again. If the problem persists, verify if your account has not been blocked.", *args, **kwargs):
